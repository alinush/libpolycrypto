...
using libpolycrypto::random_group_elems;
using libpolycrypto::random_field_elems;
...
void benchAllRootsOfUnity(size_t n, size_t r) {
    ...
    for(size_t id = 0; id < n; id++) {
        ...
        for(size_t i = 0; i < r; i++) {
            ...
            tn.startLap();
            libpolycrypto::multiExp<G1>(bases, exp);
            tn.endLap();
        }
...
int main(int argc, char *argv[]) {
    ...
    //exp = libpolycrypto::get_all_roots_of_unity(Utils::log2ceil(n));
    //exp.resize(n);
