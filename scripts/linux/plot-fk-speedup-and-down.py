...
for name in colnames:
    amt = csv_data[csv_data[proto_type] == 'AMT ' + suffix]['avg_' + name + '_usec'].values.astype(float)
    fk = csv_data[csv_data[proto_type] == 'FK ' + suffix]['avg_' + name + '_usec'].values.astype(float)

    # for e2e_bc, FK is slower, so want to print slowdown rather than speed-up
    if 'e2e_bc' in name:
        fk_speedup_cols[name] = fk / amt
    else:
        fk_speedup_cols[name] = amt / fk
...
colname_to_friendly = {
    'deal'              : 'FK ' + suffix + ' dealing speed-up', 
    'verify'            : 'FK ' + suffix + ' verify speed-up', 
    'verify_best_case'  : 'FK ' + suffix + ' verify (BC) speed-up', 
    'verify_worst_case' : 'FK ' + suffix + ' verify (WC) speed-up', 
    'reconstr_bc'       : 'FK ' + suffix + ' reconstr. (BC) speed-up', 
    'reconstr_wc'       : 'FK ' + suffix + ' reconstr. (WC) speed-up', 
    'e2e_bc'            : 'AMT ' + suffix + ' e2e best-case speed-up', 
    'e2e_wc'            : 'FK ' + suffix + ' e2e worst-case speed-up',
}
...
def plotNumbers(data, has_logscale, colNames, legendAppendix, png_name):
    ...
    ax1.set_ylabel("VSS end-to-end time speed-up")   #, fontsize=fontsize)
    ax1.grid(True)
    ...
    #    ax1.set_title(title)
    ax1.set_xticks(x)
    ax1.set_yticks([1,2,3])  # TODO: get min/max speedup from loop above
...
plotNumbers(fk_speedup, 
    ...
    colnames,
    colname_to_friendly,
    'fk-' + proto_type + '-speedup-and-down')
